...
void displayShowStatus(const char* msg, int batteryPercent) {
    ...
    char batt[20];
    snprintf(batt, sizeof(batt), "Batt: %d%%", batteryPercent);
...
void displayShowWiFiStatus(const char* wifiStatus, const char* provisioningStatus, float batteryVoltage) {
    ...
    // WiFi接続状況（1行目）
    char line1[16];
    snprintf(line1, sizeof(line1), "WiFi:%.10s", wifiStatus ? wifiStatus : "None");
    u8g2.drawStr(0, 8, line1);
    
    // プロビジョニング/認証情報送信状況（2行目）
    if (provisioningStatus) {
        char line2[16];
        snprintf(line2, sizeof(line2), "Auth:%.10s", provisioningStatus);
